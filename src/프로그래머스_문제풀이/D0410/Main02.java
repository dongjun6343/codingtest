...
//        System.out.println((s.solution("E=M*C^2","e=m*c^2")));
    ...
    public int solution(String str1, String str2) {
        ...
        for(int i = 0 ; i < chr1.length-1; i++){
            chk1 = chr1[i]+""+chr1[i+1];
            boolean matches = Pattern.matches(pattern, chk1);
            if(matches){ // true이면 알파벳만.
                hm.put(chk1, 1);
            }
        }

        for(int i = 0 ; i < chr2.length-1; i++){
            chk2 = chr2[i]+""+chr2[i+1];
            boolean matches = Pattern.matches(pattern, chk2);
            if(matches){ // true이면 알파벳만.
                hm.put(chk2, hm.getOrDefault(chk2, 0)+1);
            }
        }
        ...
        double hap = (double)count/(double)size;
        answer = (int)(hap * 65536);



//        String pattern = "^[A-Z]*$";
//        boolean chk = Pattern.matches(pattern, );
//        boolean chk2 = Pattern.matches(pattern, );
...
//반례 1
// 첫 문자열을 두 글자씩 나눠보면, [aa, a1, 1+, +a, aa, a2]로 숫자,
// 특문을 제외하면 [aa, aa]
// 마찬가지로 두번째 문자열도 해 보면 [AA, AA, AA, A1 12]로 숫자,
// 특문 제외 시 [AA, AA, AA]
// 즉, 2/3 * 65536으로 43690
...
// 잘못 생각하고 있었던 부분을 수정해서 다시 풀어보자.
// hm.getOrDefault로 하면 원하는 값이 안나오므로 수정이 필요할거같다.
