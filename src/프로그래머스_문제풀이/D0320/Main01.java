...
//할인하는 제품은 하루에 하나씩만 구매할 수 있습니다. 알뜰한 정현이는 자신이 원하는 제품과 수량이 할인하는 날짜와
...
//				new String[] {"chicken", "apple", "apple", "banana", "rice", "apple", "pork", "banana", "pork", "rice", "pot", "banana", "apple", "banana"}));
        System.out.println(s.solution(new String[] {"apple"},
                new int[] {10},
    ...
    // 할인하는 제품은 하루에 하나씩만 구매할 수 있습니다.  정현이는 자신이 원하는 제품과 수량이 할인하는 날짜에 회원.
    ...
    public int solution(String[] want, int[] number, String[] discount) {
        ...
        for(int i = 0; i<want.length; i++) {
            hm.put(want[i], number[i]); // {banana=3, apple=2, pot=1, rice=2, pork=2}
        }

        for (int i = 0;  i < discount.length; i++) {
            //{banana=4, apple=4, chicken=1, pot=1, rice=2, pork=2}
            disCountMap.put(discount[i], disCountMap.getOrDefault(discount[i], 0) + 1);
        }

        // https://hianna.tistory.com/573 확인해보기.
        // 키와 벨류가 같은값이면 ++
        Set<String> keySet = hm.keySet(); // banana, apple, pot ...

        for(String key : keySet) {
            System.out.println(disCountMap.get(key));
            System.out.println(hm.get(key));
            if(disCountMap.get(key) != null && disCountMap.get(key).equals(hm.get(key))) {
                answer++;
            }
        }
        return answer;
